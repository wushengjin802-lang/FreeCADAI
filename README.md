...
FreeCADAI 是一个面向 FreeCAD 的 AI 智能工作台原型插件。当前版本已实现阶段 3 流程：文本需求输入、三维实体/二维草图模式、常用模板、OpenAI-compatible LLM API 配置、JSON 脚本生成、安全校验、脚本预览、参数 JSON 编辑、自动修复循环、在当前 FreeCAD 文档中执行脚本，以及本地历史记录。
...
2. 选择 `FreeCADAI` 工作台。
3. 打开 AI 面板中的 `设置` 标签页。
4. 填写 `Base URL`、`API Key`、`Model` 和 `Temperature`。
5. 回到 `生成` 标签页。
6. 选择 `建模模式`：常规建模选 `三维实体`，只需要 Sketcher/Draft 轮廓时选 `二维草图`。
7. 输入建模需求，或在 `常用模板` 中选择一个模板并点击 `套用模板`。
...
8. 点击 `调用 LLM 生成脚本`。
9. 检查生成的脚本预览。
10. 检查或修改 `参数 JSON 编辑` 区域。
11. 如修改了参数，点击 `按参数重新生成`。
12. 勾选 `执行失败时自动修复`，设置最多修复次数。
13. 点击 `执行脚本`，插件会自动校验脚本，通过后再执行。
14. 确认当前 FreeCAD 文档中生成了 3D 模型或二维草图。
15. 如果自动修复仍失败，可点击 `调用 LLM 修复脚本` 手动再修复一次。
16. 打开 `历史` 标签页，确认生成、修复、自动修复、执行记录已保存。
